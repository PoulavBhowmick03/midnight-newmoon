/**
 * Deploy the owner-gated counter contract to a Midnight network
 * (undeployed by default; use --network preview|preprod for public networks).
 *
 * Adapted from the create-mn-app hello-world template, with one important
 * difference: this contract's constructor consumes a private witness
 * (the owner's secret key), so the deploy attaches real witnesses and an
 * initial private state instead of vacant ones. The secret key is generated
 * once, stored in .owner-secret (gitignored), and never leaves this machine.
 */
...
// ─── Owner secret key (private witness input) ─────────────────────────────────
//
// 32 random bytes. Only its hash commitment ever reaches the chain.

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ownerSecretPath = path.resolve(__dirname, '..', '.owner-secret');

function getOrCreateOwnerSecret(): Uint8Array {
  if (fs.existsSync(ownerSecretPath)) {
    return Uint8Array.from(Buffer.from(fs.readFileSync(ownerSecretPath, 'utf8').trim(), 'hex'));
  }
  const key = crypto.randomBytes(32);
  fs.writeFileSync(ownerSecretPath, key.toString('hex') + '\n', { mode: 0o600 });
  console.log(`  Generated new owner secret key → .owner-secret (keep it private!)`);
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy owner-gated counter to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');

  const seed = SEED;
  const ownerSecretKey = getOrCreateOwnerSecret();
  ...
  // DUST balance is a time-projection; give it a block before the first try.
  ...
  const initialPrivateState: CounterPrivateState = { secretKey: ownerSecretKey };
